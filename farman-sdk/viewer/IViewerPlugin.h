...
//
//   // metadata.json
//   { "MinHostVersion": "0.9.9" }
//
// farman は各プラグインをロードする「前」に QPluginLoader::metaData() で
// MetaData.MinHostVersion を読み、本体バージョン (数値部のみで比較、prerelease は
// 無視) がそれ未満なら instance() を呼ばずにスキップし、Settings > Plugins に
// 「要 farman X 以降」と表示する。バイナリが依存不足で dlopen 失敗するケースでも、
// メタデータはコード解決前に読めるため、原因の分かる形で弾ける。宣言が無ければ
// 従来どおり無条件にロードを試みる。実行時のより細かい判定には PluginContext の
// farmanVersion も使える。
